---
date: "2026-07-18T11:50:00+08:00"
author: skillre
commit: d37c3d9
branch: main
repository: 数字展厅
topic: "视觉重做·深蓝赛博科技数据风"
tags: [design, visual-redesign, threejs, cyber-blue, tech-aesthetic]
status: ready
parent: .rpiv/artifacts/visual-reviews/2026-07-18_digital-exhibition-hall-visual-review.md
last_updated: "2026-07-18T11:50:00+08:00"
last_updated_by: skillre
---

# Design: 视觉重做·深蓝赛博科技数据风

## Summary

将数字展厅 3D 视觉从"传统拟物写实"（大理石/木材/盆栽/白墙）整体重做为"深蓝赛博科技数据风"：深蓝黑基调 + 青色霓虹强调，深色镜面反射地面 + 发光网格，半透明玻璃数据屏展板，中央盾牌全息 + 数据流粒子环标志装置，HUD 发光标签替代 emoji 白底招牌，中度环境动效。3D 与 UI 统一到同一套深蓝赛博 design tokens，使其具备科技感、数据感、安全感，达到可交付水准。

## Requirements

- 风格基调：深蓝赛博 Cyber Blue（用户确认）
- 色板：基调 `#050d1f` + 强调 `#00d2ff` + 辅 `#0066ff` + 点缀 `#00ff88`(安全)/`#ff00aa`(威胁)（用户确认）
- 地面：深色镜面反射 + 青色发光网格线（用户确认）
- 墙面：暗锐蓝 `#0d1117` + 嵌入式发光条
- 展板：半透明玻璃数据屏 + 发光边框 + CanvasTexture 缩略图/标题（用户确认）
- 中央装置：盾牌全息 + 数据流粒子环（用户确认）
- 现有家具：全部移除（盆栽/长椅/壁灯/木质前台/地垫/门把手/踢脚线/墙裙）（用户确认）
- 动效：中度（数据流粒子 + 网格流光 + 呼吸灯 + 悬浮微动 + 轻扫描线）（用户确认）
- HUD 字体：系统等宽/无衬线，零依赖离线可用（用户确认）
- 标签：深色半透明 + 青色发光描边 + 矢量图标替代 emoji
- 性能：60fps（GTX 1060 级别），加载 < 3 秒（继承需求文档）
- 部署：纯静态，可离线 / GitHub Pages（继承需求文档）

## Current State Analysis

### Key Discoveries
- `js/config.js:8` 背景 `0xd0d4dc` 浅灰、`:10` 雾 `0xd0d4dc` 浅灰 —— 整屏浅灰，无科技感
- `js/objects/ExhibitionHall.js` 全文为拟物写实：大理石 `drawMarbleTexture`(:126)、白墙 `drawWallTexture`(:144)、方格天花 `drawCeilingTexture`(:156)、木材 `materials.wood`(:191)、盆栽 `createPottedPlants`(:391)、长椅 `createBenches`(:363)、壁灯 `createWallSconces`(:416)
- `ExhibitionHall.js:847` `getTypeIcon` 用 emoji `📄🖼️🎬📊🧊`；`createTextSprite`(:770) 生成白底圆角招牌 —— 简陋感主源
- `js/scene/SceneManager.js:118-130` 仅 Bloom strength 0.15（极轻）；design 文档提的 SSAO 未实现
- `css/style.css:20` body `#1a1a2e`、主色 `#00d2ff` —— UI 已是深色赛博风，与 3D 浅灰割裂
- `package.json` 构建链 `rollup-darwin-arm64` 原生模块损坏导致 `vite build` 失败（已临时 `npm install` 修复）

### Patterns to Follow（可复用视觉构建工具）
- `ExhibitionHall.createProceduralTexture()` at `ExhibitionHall.js:103` — CanvasTexture 程序纹理生成模式（将用于科技网格/数据流纹理）
- `ExhibitionHall.createTextSprite()` at `ExhibitionHall.js:770` — CanvasTexture 标签模式（将改造为 HUDLabel）
- `ExhibitionHall.createEntranceParticles()` at `ExhibitionHall.js:693` — Points 粒子系统模式（将扩展为数据流粒子）
- `ExhibitionHall._textures/_geometries/_trackedMaterials` 资源追踪模式
- `SceneManager.createPostProcessing()` at `SceneManager.js:118` — EffectComposer + UnrealBloomPass 管线
- `DataDashboard.drawChart()` at `DataDashboard.js:50` — CanvasTexture 图表模式

### Constraints
- Three.js r158（npm），可用 `three/examples/jsm/` 的 `Reflector`、`EffectComposer`、`UnrealBloomPass`、`RoomEnvironment`
- 纯静态前端，无后端；可离线部署 → 不依赖联网字体/CDN
- 程序化生成材质纹理（CanvasTexture），不加载外部贴图文件
- 60fps 性能目标 → 反射/Bloom/粒子需节制

## Scope

### Building
- Design tokens 单一来源（JS CONFIG + CSS 变量）
- 深蓝赛博色彩基调（背景/雾/灯光/曝光）
- 后处理强化（Bloom 提强、暗色环境贴图、FogExp2）
- 科技空间骨架（镜面反射地面+发光网格、暗锐蓝墙面+发光条、暗色天花板+灯带）
- 移除全部写实家具
- 中央盾牌全息 + 数据流粒子环装置（新增 TechCenterpiece）
- HUD 标签系统（新增 HUDLabel，替代白底招牌/emoji）
- 展板玻璃数据屏 + 发光边框 + 缩略图标题
- 中度环境动效（数据流粒子、网格流光、呼吸灯、悬浮微动、轻扫描线）
- 数据可视化科技样式升级
- UI 统一深蓝赛博（导航/模态框/加载界面 + design tokens + 矢量图标）

### Not Building
- 移动端触屏适配改造（继承既有 defer）
- 多人协作 / 真实 API 对接 / 语音导览（继承既有 defer）
- 外部 glTF/GLB 模型加载（继续程序化生成）
- SSAO（性能权衡，本轮用 Bloom + 反射营造深度，SSAO 留待后续）
- 联网字体（离线部署约束，用系统字体）

## Decisions

### D1: 风格基调 = 深蓝赛博 Cyber Blue
- **Ambiguity**: 传统拟物写实 vs 科技数据风（3 个变体）
- **Explored**: 深蓝赛博（蓝=信任/安全/科技，最贴主题）/ 暗夜数据流（绿=运行状态，偏 SOC）/ 深空全息（紫=加密，未来感强但安全联想弱）
- **Decision**: 深蓝赛博
- **Rationale**: 用户确认；蓝色天然关联信任/安全/科技，最贴合"数据安全服务"主题，安全作战室/科幻指挥中心意象

### D2: 色板
- **Decision**: 基调 `#050d1f` 深蓝黑（背景/雾）、强调 `#00d2ff` 青色霓虹（灯带/线框/数据点）、辅 `#0066ff` 冰蓝（全息投影）、点缀 `#00ff88` 翠绿（安全状态）/ `#ff00aa` 品红（威胁）；墙面 `#0d1117` 暗锐蓝
- **Rationale**: 用户确认 + web research 参考（Cyberpunk/Tron 色板）

### D3: 地面 = 深色镜面反射 + 青色发光网格
- **Ambiguity**: 镜面反射+网格（炫，性能中）/ 哑光+网格（稳，性能好）/ 纯网格（轻，质感弱）
- **Decision**: 镜面反射 + 发光网格（Three.js Reflector）
- **Rationale**: 用户确认；科技感最强，中端显卡可胜任 60fps

### D4: 展板 = 半透明玻璃数据屏 + 发光边框 + CanvasTexture 缩略图标题
- **Decision**: MeshPhysicalMaterial 半透明玻璃 + EdgesGeometry 发光边框 + CanvasTexture 缩略图/标题
- **Rationale**: 用户确认；与现有 createPanel 结构兼容，改造代价小

### D5: 中央装置 = 盾牌全息 + 数据流粒子环
- **Decision**: 新增 TechCenterpiece —— 悬浮盾牌全息（自发光+线框）+ 外围旋转数据流粒子环
- **Rationale**: 用户确认；象征"防护屏障"，最贴数据安全主题

### D6: 现有家具全部移除
- **Decision**: 删除盆栽/长椅/壁灯/木质前台/地垫/门把手/踢脚线/墙裙；入口改科技门框
- **Rationale**: 用户确认；纯科技抽象空间最干净

### D7: 动效 = 中度
- **Decision**: 数据流粒子 + 地面网格流光 + 呼吸灯 + 悬浮微动 + 轻扫描线
- **Rationale**: 用户确认；"活着"但不花哨，性能可控

### D8: HUD 字体 = 系统字体
- **Decision**: 系统等宽（SF Mono/Menlo/Consolas）+ 无衬线，零依赖离线可用
- **Rationale**: 用户确认；项目要内网/离线部署，CDN 字体会失效

### D9: 标签 = HUD 发光标签替代 emoji 白底招牌
- **Decision**: 新增 HUDLabel 工具 —— 深色半透明 + 青色发光描边 + 矢量线图标（Canvas 绘制）替代 emoji；替换所有 createTextSprite 白底招牌
- **Rationale**: 用户确认；消除简陋感主源（emoji+白底）

## Architecture

> 代码切片在 Step 6 逐片填入。占位代码块在生成后由 Edit 替换为完整实现。
...
> 最终合并版（Slice 1 tokens + Slice 7 UI 统一深蓝赛博）。
...
> 多 slice 贡献：Slice 3（骨架）/ 4（家具+装置）/ 5（标签+展板）/ 6（动效）。按 `// ===== Slice N =====` 分段。
...
// --- 改动6.2：createCeilingLights() _ceilingBars 接线（已在 Slice 3 改动3.6 内实现，此处无需重复）---
...
### js/objects/TechCenterpiece.js — NEW
...
### js/objects/DataDashboard.js — MODIFY
...
> Slice 4 装配 TechCenterpiece；Slice 6 钩入环境动效。
...
### Slice 1: Design Tokens 与色彩基调
**Files**: `js/config.js`, `css/style.css`

#### Automated Verification:
- [ ] 浅灰基调已移除：`grep -n "0xd0d4dc" js/config.js` 无输出
- [ ] JS tokens 已建立：`grep -n "export const THEME" js/config.js` 返回 1
- [ ] CSS tokens 已建立：`grep -n "--neon" css/style.css` 返回 >= 1
- [ ] 构建通过：`npm run build`（先 `rm -rf node_modules && npm install` 修复 rollup）
...
### Slice 2: 场景渲染基调
**Files**: `js/scene/SceneManager.js`

#### Automated Verification:
- [ ] RoomEnvironment 引用已移除：`grep -n "RoomEnvironment" js/scene/SceneManager.js` 无输出
- [ ] THEME 已引入：`grep -n "import { THEME }" js/scene/SceneManager.js` 返回 1
- [ ] FogExp2 已用：`grep -n "FogExp2" js/scene/SceneManager.js` 返回 1
- [ ] Bloom 参数来自 THEME：`grep -n "THEME.bloom" js/scene/SceneManager.js` 返回 >= 1
- [ ] 构建通过：`npm run build`
...
### Slice 3: 科技空间骨架
**Files**: `js/objects/ExhibitionHall.js`

#### Automated Verification:
- [ ] 写实纹理已删：`grep -n "drawMarbleTexture\|drawWallTexture\|drawCeilingTexture" js/objects/ExhibitionHall.js` 无输出
- [ ] 墙裙/踢脚线已删：`grep -n "createBaseboard\|createCrownMolding\|createWainscoting" js/objects/ExhibitionHall.js` 无输出
- [ ] Reflector 已用：`grep -n "Reflector" js/objects/ExhibitionHall.js` 返回 >= 1
- [ ] 网格地面已用：`grep -n "GridHelper" js/objects/ExhibitionHall.js` 返回 1
- [ ] 构建通过：`npm run build`
...
### Slice 4: 移除家具 + 中央装置
**Files**: `js/objects/ExhibitionHall.js`, `js/objects/TechCenterpiece.js`, `js/app.js`

#### Automated Verification:
- [ ] 家具已删：`grep -n "createPottedPlants\|createBenches\|createWallSconces\|createReceptionDesk\|createInfoKiosks" js/objects/ExhibitionHall.js` 无输出
- [ ] TechCenterpiece 已建：`grep -n "class TechCenterpiece" js/objects/TechCenterpiece.js` 返回 1
- [ ] app.js 已装配：`grep -n "TechCenterpiece" js/app.js` 返回 >= 2
- [ ] 渲染循环调用装置 update：`grep -n "techCenterpiece.update" js/app.js` 返回 1
- [ ] 构建通过：`npm run build`
...
### Slice 5: HUD 标签 + 展板科技化
**Files**: `js/objects/HUDLabel.js`, `js/objects/ExhibitionHall.js`

#### Automated Verification:
- [ ] emoji 已移除：`grep -rn "📄\|🖼️\|🎬\|📊\|🧊" js/` 无输出
- [ ] getTypeIcon 已删：`grep -n "getTypeIcon" js/objects/ExhibitionHall.js` 无输出
- [ ] HUDLabel 已建：`grep -n "class HUDLabel" js/objects/HUDLabel.js` 返回 1
- [ ] 矢量图标已用：`grep -n "drawIcon" js/objects/HUDLabel.js` 返回 >= 1
- [ ] 展板玻璃屏：`grep -n "isBoard" js/objects/ExhibitionHall.js` 返回 >= 1
- [ ] 构建通过：`npm run build`
...
#### Automated Verification:
- [ ] 粒子青色：`grep -n "THEME.neon" js/objects/ExhibitionHall.js` 返回 >= 1（含粒子）
- [ ] 环境动效方法：`grep -n "updateAmbience" js/objects/ExhibitionHall.js` 返回 1
- [ ] 渲染循环传 elapsed：`grep -n "updateParticles(sceneManager.clock.getElapsedTime())" js/app.js` 返回 1
- [ ] 构建通过：`npm run build`
...
### Slice 7: 数据可视化 + UI 统一收尾
**Files**: `js/objects/DataDashboard.js`, `css/style.css`, `css/ui.css`

#### Automated Verification:
- [ ] 图表科技样式：`grep -n "#0a1628\|DATA METRICS" js/objects/DataDashboard.js` 返回 >= 1
- [ ] style.css 全量 var：`grep -c "var(--" css/style.css` 返回 >= 20
- [ ] ui.css 全量 var：`grep -c "var(--" css/ui.css` 返回 >= 15
- [ ] 旧硬编码青色已清理：`grep -n "#1a1a2e" css/style.css` 无输出
- [ ] 构建通过：`npm run build`（先 `rm -rf node_modules && npm install`）
- [ ] 项目级无残留写实：`grep -rn "0xd0d4dc\|大理石\|marble\|wood\|盆栽\|plant\|bench\|sconce" js/ css/` 无输出
- [ ] 项目级无残留 emoji：`grep -rn "📄\|🖼️\|🎬\|📊\|🧊" js/` 无输出
...
- `js/config.js`  # MODIFY — 深蓝赛博色板/灯光/雾/材质参数 design tokens
- `css/style.css`  # MODIFY — CSS design tokens 变量 + UI 深蓝赛博统一
- `css/ui.css`  # MODIFY — UI 组件科技样式收尾
- `js/scene/SceneManager.js`  # MODIFY — 暗冷灯光/暗色环境贴图/Bloom 提强/FogExp2
- `js/objects/ExhibitionHall.js`  # MODIFY — 科技空间骨架/移除家具/展板科技化/动效
- `js/objects/TechCenterpiece.js`  # NEW — 中央盾牌全息 + 数据流粒子环装置
- `js/objects/HUDLabel.js`  # NEW — HUD 发光标签工具（替代白底招牌/emoji）
- `js/objects/DataDashboard.js`  # MODIFY — 数据可视化科技样式
- `js/app.js`  # MODIFY — 装配 TechCenterpiece + 动效钩入渲染循环

## Ordering Constraints

- Slice 1（Tokens）必须最先 —— 所有切片引用色板/参数
- Slice 2（渲染基调）依赖 Slice 1 的 config
- Slice 3（空间骨架）依赖 Slice 1,2
- Slice 4（装置）依赖 Slice 3
- Slice 5（标签展板）依赖 Slice 1,3
- Slice 6（动效）依赖 Slice 3,4
- Slice 7（数据可视化+UI）依赖 Slice 1,5
- Slice 1→2→3 严格顺序；4/5 可在 3 后并行（但按序生成避免冲突）；6 依赖 4；7 收尾

## Verification Notes

- 构建校验：`npm run build` 干净通过（先 `rm -rf node_modules && npm install` 修复 rollup 原生模块）
- 运行校验：`npm run dev` 启动，浏览器无 console error
- 视觉校验：Playwright 截图，3D 画布主色应为深蓝黑系（非浅灰 `(168,170,174)`），可见青色发光元素
- 性能校验：60fps（Chrome Performance 面板），反射/Bloom 开启下中端显卡达标
- grep 校验：`grep -rn "0xd0d4dc\|大理石\|marble\|wood\|盆栽\|plant\|bench\|sconce" js/ css/` 应无残留写实材质/家具
- grep 校验：`grep -rn "📄\|🖼️\|🎬\|📊\|🧊" js/` 应无 emoji 残留
- design tokens 单一来源：色值在 config.js（JS）与 :root 变量（CSS）两处定义，需保持一致
...
- `ExhibitionHall.createProceduralTexture()` `ExhibitionHall.js:103` — 程序纹理模式（科技网格/数据流）
- `ExhibitionHall.createTextSprite()` `ExhibitionHall.js:770` — 标签模式（改造为 HUDLabel）
- `ExhibitionHall.createEntranceParticles()` `ExhibitionHall.js:693` — 粒子模式（扩展数据流）
- `SceneManager.createPostProcessing()` `SceneManager.js:118` — 后处理管线
- `three/examples/jsm/objects/Reflector` — 镜面反射地面
- `three/examples/jsm/postprocessing/UnrealBloomPass` — Bloom

## Developer Context

**Q (视觉基调)**: 整体风格基调选哪个？
A: 深蓝赛博 Cyber Blue — 基调 #050d1f + 强调 #00d2ff + 辅 #0066ff + 点缀 #00ff88/#ff00aa（证据：visual-review V1-01 风格错位；web research 色板）

**Q (现有家具)**: 盆栽/长椅/壁灯/木质前台等怎么处理？
A: 全部移除（证据：ExhibitionHall.js createPottedPlants/createBenches/createWallSconces）

**Q (中央装置)**: 视觉锚点选哪个？
A: 盾牌全息 + 数据流粒子环（证据：web research 标志装置；数据安全主题"防护屏障"意象）

**Q (地面)**: 地面怎么处理？
A: 镜面反射 + 发光网格（证据：web research 地面处理；Three.js Reflector）

**Q (展板形态)**: 展板选哪个？
A: 半透明玻璃数据屏 + 发光边框 + 缩略图标题（证据：ExhibitionHall.js createPanel 结构兼容）

**Q (动效强度)**: 常驻动效选哪个？
A: 中度（证据：web research 动效；性能 60fps 约束）

**Q (HUD字体)**: 字体方案选哪个？
A: 系统字体零依赖（证据：需求文档离线/GitHub Pages 部署约束）

**Q (展板内容)**: 展板表面怎么呈现？
A: 缩略图+标题 CanvasTexture（证据：DataDashboard CanvasTexture 模式可复用）
...
- Slice 1: Design Tokens 与色彩基调 — approved as generated
- Slice 2: 场景渲染基调 — approved as generated
- Slice 3: 科技空间骨架 — approved as generated
- Slice 4: 移除家具 + 中央装置 — approved as generated
- Slice 5: HUD 标签 + 展板科技化 — approved as generated
- Slice 6: 环境动效系统 — approved as generated
- Slice 7: 数据可视化 + UI 统一收尾 — approved as generated
...
- web research 科技风参考：Cyberpunk/Tron 色板、Three.js Reflector/Bloom、Orbitron/JetBrains Mono
